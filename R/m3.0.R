...
#' @export
calcR0.3.0 <- function(beta = 0.75, gamma1 = 1/4)
{
...
#' @export
rmeas3.0 <- function(newI,MSIf,MSIg,MIh,seedrate,N,beta,delta, ...){
	rf <- beta * 1.5/7# factor 1.5/7 is act rate per day; factor 1.5 b/c more contacts per week in long partnerships 
	rg <- beta * 1/7
...
#' @export 
rinit3.0 <- function(i0,N,seedrate0, ... ){
	xinit <- i0 / N 
	c(
		thetaf = 1 
			, MSEf = 0
...
m3.0 <- pomp::pomp(
	...
	, data = NULL
	, times = seq( 1, 90 ) 
	, rprocess = pomp::discrete_time(step.fun = rstep3.0)
	, rmeasure = rmeas3.0
	, dmeasure = dmeas3.0
	, rinit = rinit3.0
	, statenames = c('thetaf'
